...
namespace BooksTextsSplit.Library.Services
{
    ...
    public class ControllerCacheManager : IControllerCacheManager
    {
        ...
        public async Task<(string, int)> CreateTestScenarioKey(ConstantsSet constantsSet, int testScenario)
        {
            ...
            bool testSettingKey1WasDeleted = await _access.RemoveWorkKeyOnStart(testScenarioSequenceKey);
            ...
            await _access.WriteHashedAsync<int, int>(testScenarioSequenceKey, fieldValues, testScenarioSequenceKeyLifeTime);

            IDictionary<int, int> fieldValuesResult = await _access.FetchHashedAllAsync<int, int>(testScenarioSequenceKey);
